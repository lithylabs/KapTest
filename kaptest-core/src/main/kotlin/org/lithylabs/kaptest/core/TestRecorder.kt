...
class TestRecorder (
    ...
    fun findClassMethodNames(ignoreClasses: List<String>): Pair<String, String> {
        ...
        var className = stackElement.className
        var methodName = stackElement.methodName
        if (methodName.startsWith("invokeSuspend")) {
            val dollarSplit = stackElement.className.split("$")
            className = dollarSplit[0]
            methodName = dollarSplit[1]
        }
